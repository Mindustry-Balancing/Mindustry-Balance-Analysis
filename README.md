# Mindustry Balance Analysis
[Made by Someone's Shadow]

A repository made to document problem sets, mechanics, and specific analysis for referencing in suggestions, prs, or general discussion. A repository used in #balancing in The Official Mindustry Discord Server.

...It's still in the works, todos, and whatnot, so its not finished and mainly empty

Yeah this is mostly in the works but ill try to finish it

## Navigation
- **Methods of Analysis** [🔗](https://github.com/Mindustry-Balancing/Mindustry-Balance-Analysis/blob/main/Methods%20of%20Analysis/Analysis%20Overview.md)
  - How to do balancing: from analysis to implementation.
- **Terminology** [🔗](https://github.com/Mindustry-Balancing/Mindustry-Balance-Analysis/blob/main/Terminology.md)
  -  A Wiki of sorts for terms I do use for balancing or talking about the game in general.

## TODO
(not done yet)
- **Serpulo Content Analysis** [🔗](https://github.com/Mindustry-Balancing/Mindustry-Balance-Analysis/blob/main/Serpulo/Content%20Specific%20Analysis.md)
  -  Analyzing specific Serpulo Content.
